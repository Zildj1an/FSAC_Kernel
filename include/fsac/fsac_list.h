...
extern void fsac_remove_list(struct list_head* ghost_node);

/* Return value = number of elements copied into members[] */
extern int fsac_print_list(struct list_head* list, char* members);
...
*/
extern struct list_head* fsac_find_node(int n, char *c, struct list_head* head);

/* Safely copying contents to user array */
extern int fsac_copy_safe(char *kbuf, unsigned long ksize,__user const char* ubuf,
